...
function pdfSafeInline(value) {
  return pdfSafeText(value).replace(/\n+/g, " ").trim();
}

function pdfSafeHabitName(name) {
  return pdfSafeInline(name || "Untitled Habit");
}
...
function formatShortGoalTypeLabel(targetType, targetValue) {
  ...
  if (safeType === "weekly") return `${safeValue}x/week`;
  if (safeType === "monthly") return `${safeValue}x/month`;
...
function formatPdfGoalTypeLabel(targetType, targetValue) {
  ...
  if (safeType === "weekly") return `Weekly - ${safeValue}x/week`;
  if (safeType === "monthly") return `Monthly - ${safeValue}x/month`;
...
function getScaleStatus(value, targetLow, targetHigh) {
  if (value < targetLow) {
    return { label: "Below Target", textColor: [220, 38, 38] };
  }

  if (value <= targetHigh) {
    return { label: "On Target", textColor: [22, 163, 74] };
  }

  return { label: "Above Target", textColor: [234, 88, 12] };
}

function drawActiveMarkdownNotesCardPage(doc) {
  const context = doc.__activeMarkdownNotesCard;

  if (!context) return null;

  const pageHeight = doc.internal.pageSize.getHeight();
  const { x, w, margin, padding } = context;
  ...
  doc.roundedRect(x, cardY, w, cardH, 5, 5, "FD");
...
function ensurePageSpace(doc, currentY, neededHeight, margin = 12) {
  ...
  if (currentY + neededHeight > pageHeight - margin) {
    ...
    const markdownCardStartY = drawActiveMarkdownNotesCardPage(doc);

    return markdownCardStartY ?? margin;
  }
...
function drawSectionTitle(doc, title, subtitle, y, pageWidth, margin) {
  const safeTitle = pdfSafeInline(title);
  const safeSubtitle = pdfSafeInline(subtitle);
...
function drawCard(
  ...
  const safeTitle = pdfSafeInline(title);
  const safeValue = pdfSafeInline(value);
  const safeSubtitle = pdfSafeInline(subtitle);
  ...
  doc.text(safeTitle, x + 4, y + 4.8);

  doc.setFont("helvetica", "bold");
  doc.setFontSize(isCompact ? 13 : 15);
  doc.setTextColor(...valueColor);
  doc.text(safeValue, x + 4, isCompact ? y + 11.2 : y + 13);
  ...
  if (safeSubtitle) {
    ...
    doc.text(
      doc.splitTextToSize(safeSubtitle, w - 8).slice(0, 1),
      x + 4,
      y + h - 2.8,
    );
  }
...
function drawStatusPill(doc, x, y, text, fillColor, textColor) {
  const safeText = pdfSafeInline(text);
  const paddingX = 4.5;
  const pillH = 7;
  const pillW = doc.getTextWidth(safeText) + paddingX * 2;
...
function drawHighlightCard(
  doc,
  { x, y, w, h, title, mainValue, subtitle, progress = 0 },
) {
  const theme = getProgressTheme(progress);
  const safeTitle = pdfSafeInline(title);
  const safeMainValue = pdfSafeHabitName(mainValue || "-");
  const safeSubtitle = pdfSafeInline(subtitle || "No data available");
  const padX = 6;
  ...
  const pillW = doc.getTextWidth(theme.label) + 9;
  drawStatusPill(
    doc,
    x + w - pillW - padX,
    y + 4.5,
  ...
  doc.setTextColor(100, 116, 139);
  doc.text(
    doc.splitTextToSize(safeSubtitle, w - padX * 2).slice(0, 2),
    x + padX,
    y + 25,
  );
}
...
function drawProgressBar(doc, x, y, w, h, label, value) {
  const safeLabel = pdfSafeInline(label);
...
function drawReviewBox(doc, { x, y, w, h, title, text }) {
  const safeTitle = pdfSafeInline(title);
  const safeTextValue = pdfSafeInline(text || "No entry");
  ...
  doc.setTextColor(100, 116, 139);
  doc.text(
    doc.splitTextToSize(safeTextValue, w - 8).slice(0, 5),
    x + 4,
    y + 12,
  );
}
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (/^```/.test(line)) {
      if (inCodeBlock) {
        const codeWrappedLines = codeBuffer.flatMap((codeLine) =>
          doc.splitTextToSize(pdfSafeText(codeLine), w - 10),
        );
    ...
    if (quoteMatch) {
      const wrapped = doc.splitTextToSize(
        stripInlineMarkdown(quoteMatch[1]),
        w - 14,
      );
      height += Math.max(12, wrapped.length * 4.2 + 7) + 4;
      continue;
    }
    ...
    const text =
      taskMatch?.[2] || unorderedMatch?.[1] || orderedMatch?.[2] || line;
    const wrapped = doc.splitTextToSize(
      stripInlineMarkdown(text),
      w - (taskMatch || unorderedMatch || orderedMatch ? 8 : 0),
    );
    height += wrapped.length * 4.4 + 1.5;
  }
...
function drawMarkdownCodeBlock(doc, { lines, x, y, w, margin }) {
  ...
  const wrapped = codeLines.flatMap((line) =>
    doc.splitTextToSize(pdfSafeText(line), w - 10),
  );
...
function drawMarkdownNotes(doc, { notes, x, y, w, margin }) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (/^(-{3,}|\*{3,}|_{3,})$/.test(line)) {
      currentY = drawMarkdownHorizontalRule(doc, { x, y: currentY, w, margin });
      continue;
    }
...
function drawMarkdownNotesCard(doc, { notes, x, y, w, margin }) {
  ...
  const previousMarkdownCardContext = doc.__activeMarkdownNotesCard;

  doc.__activeMarkdownNotesCard = {
    x,
    w,
    margin,
    padding,
  ...
  let endY = y + boxHeight;

  try {
    endY = drawMarkdownNotes(doc, {
      notes,
      x: innerX,
      y: innerY,
      w: innerW,
    ...
    if (previousMarkdownCardContext) {
      doc.__activeMarkdownNotesCard = previousMarkdownCardContext;
    } else {
      delete doc.__activeMarkdownNotesCard;
    }
  }
...
function getTrendStats(dailyProgress) {
  if (!dailyProgress?.length) {
    return { startAvg: 0, endAvg: 0, delta: 0, bestDay: null };
  }
  ...
  return { startAvg, endAvg, delta: endAvg - startAvg, bestDay };
}
...
function drawComparisonMetricCard(
  ...
  const safeTitle = pdfSafeInline(title);
  const safeCurrentValue = pdfSafeInline(
    `${currentValue}${currentValue !== null && currentValue !== undefined ? suffix : ""}`,
  );
  const safePreviousValue =
    previousValue === null || previousValue === undefined
      ? "Previous: No data"
      : pdfSafeInline(`Previous: ${previousValue}${suffix}`);
  const safeDeltaText = pdfSafeInline(formatDeltaText(delta, digits, suffix));
  const padX = 6;
  ...
  doc.roundedRect(pillX, y + 4, pillW, 7, 3.5, 3.5, "F");

  doc.setFont("helvetica", "bold");
  doc.setFontSize(7.5);
  doc.setTextColor(...theme.deltaText);
  doc.text(safeDeltaText, pillX + 5, y + 8.8);
...
function drawYearlyMiniBarChart(doc, { x, y, w, h, data, currentMonth }) {
  ...
  visibleData.forEach((item, index) => {
    const value = clampNumber(item.completionPercent, 0, 100);
    const barHeight = item.isEmpty ? 2 : Math.max(4, (value / 100) * chartH);
    ...
    if (item.isEmpty) doc.setFillColor(212, 212, 212);
    else if (isCurrent) doc.setFillColor(109, 40, 217);
    else doc.setFillColor(167, 139, 250);

    doc.roundedRect(barX, barY, barW, barHeight, 1.5, 1.5, "F");

    if (!item.isEmpty) {
      points.push({ x: barX + barW / 2, y: barY, value });
    }
    ...
    doc.setTextColor(148, 163, 184);
    doc.text(
      pdfSafeInline(item.shortMonth || "").slice(0, 3),
      barX + barW / 2,
      y + h - 4,
      {
        align: "center",
...
function drawSparklineCard(doc, { x, y, w, h, title, data }) {
  ...
  const points = values.map((value, index) => ({
    ...
    y: chartY + chartH - ((value - minValue) / range) * chartH,
  }));
...
function drawStatusScaleCard(
  ...
  const safeTitle = pdfSafeInline(title);
  const safeDisplayValue = pdfSafeInline(displayValue);
  ...
  doc.text(pdfSafeInline(status.label), x + w - 4, y + 5, { align: "right" });
  ...
  if (lowRatio > 0)
    doc.roundedRect(barX, barY, barW * lowRatio, barH, 1.5, 1.5, "F");
  ...
  if (highRatio > 0) {
    doc.roundedRect(
      barX + barW * lowRatio + barW * midRatio,
      barY,
      barW * highRatio,
      barH,
      1.5,
  ...
  doc.setFontSize(6.8);
  doc.setTextColor(148, 163, 184);
  doc.text(pdfSafeInline(lowLabel), barX, barY + 7);
  doc.text(pdfSafeInline(midLabel), barX + barW / 2, barY + 7, {
    align: "center",
  });
  doc.text(pdfSafeInline(highLabel), barX + barW, barY + 7, { align: "right" });
}
...
export function exportDashboardPdf(summary) {
  ...
  const previousMonthLabel = pdfSafeInline(
    summary.previousMonthLabel || "previous month",
  );
  const safeMonth = pdfSafeInline(summary.month);
  const safeYear = pdfSafeInline(summary.year);
  const safeMonthKey = pdfSafeInline(summary.monthKey);
  ...
  doc.text(
    `Generated: ${pdfSafeInline(new Date().toLocaleDateString())}`,
    pageWidth - margin,
    y + 6,
    {
      align: "right",
  ...
  drawCard(doc, {
    ...
    h: achievementSummaryCardH,
    title: "Completion",
    value: `${Math.round((unlockedAchievements.length / Math.max(pdfAchievements.length, 1)) * 100)}%`,
  ...
  autoTable(doc, {
    ...
    body: pdfAchievements.map((achievement) => [
      pdfSafeInline(achievement.title),
      achievement.unlocked ? "Unlocked" : "Locked",
      `${achievement.progress}%`,
      pdfSafeInline(achievement.description),
    ]),
    theme: "grid",
    headStyles: {
      fillColor: [17, 24, 39],
  ...
  y = drawSectionTitle(
    doc,
    "Yearly Overview",
    `12-month summary for ${pdfSafeInline(summary.year)}`,
    y,
    pageWidth,
    margin,
  );
  ...
  drawCard(doc, {
    ...
    value: yearlyStats.bestMonth
      ? `${pdfSafeInline(yearlyStats.bestMonth.month)} ${yearlyStats.bestMonth.completionPercent}%`
      : "-",
    subtitle: yearlyStats.worstMonth
      ? `Worst: ${pdfSafeInline(yearlyStats.worstMonth.month)} ${yearlyStats.worstMonth.completionPercent}%`
  ...
  const minimumNotesStartHeight = 45;
  const remainingNotesSpace = pageHeight - margin - y;

  if (remainingNotesSpace < minimumNotesStartHeight) {
    doc.addPage();
    y = margin;
  }
